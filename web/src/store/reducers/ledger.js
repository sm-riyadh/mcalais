...
const ledger = (state = initialState, action) => {
	switch (action.type) {
		...
		case LEDGER.NEW_SAVE:
			...
			newState.filter(e => e.code === action.data.destination)[0].transactions.unshift({
				journalEntry: state.filter(e => e.code === action.data.destination)[0].transactions.length + 1,
			...
			newState.filter(e => e.code === action.data.source)[0].transactions.unshift({
				journalEntry: state.filter(e => e.code === action.data.source)[0].transactions.length + 1,
				date: new Date().getTime(),
